...
const ThemeSwitch = ({ isDark, onToggle }) => {
  return (
    <>
      {/* Desktop: full animated toggle */}
      <label className="theme-switch hidden sm:inline-flex">
        <input 
          type="checkbox" 
      ...
      <button
        ...
        onClick={onToggle}
        className={`sm:hidden w-9 h-9 rounded-full flex items-center justify-center transition-all duration-300 active:scale-90 shadow-md ${
          isDark
            ? 'bg-[#1e3246] border border-white/15 text-[#ffd97d]'
            : 'bg-[#3D7EAE] border border-white/20 text-white'
        }`}
      >
        {isDark ? (
          /* Moon icon */
